...
function buildDiagnosisSummary(diagnosis) {
  if (!diagnosis || !Array.isArray(diagnosis.questions) || !Array.isArray(diagnosis.answers)) {
    return "No diagnostic answers were provided.";
  }

  return diagnosis.questions
    .map((question, index) => `Q${index + 1}: ${question}\nA${index + 1}: ${diagnosis.answers[index] || "No answer"}`)
    .join("\n\n");
}
...
app.post("/api/chat", async (req, res) => {
  try {
    const { name, diagnosis, messages } = req.body;
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        ...
        {
          role: "system",
          content: `User name: ${name || "Unknown"}\n\nFinancial/spiritual diagnostic:\n${diagnosisSummary}`
        },
        ...safeMessages
      ]
    });
